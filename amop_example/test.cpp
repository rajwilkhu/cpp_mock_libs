...
TEST(DeviceResourceNetworkRepository, GivenANetworkRepositoryWithResources_WhenRequested_ThenShouldReturnList)
{
	...
	amop::TMockObject<IHttpRestClient> mockHttpClient;
	
	DeviceResourceNetworkRepository repository(*((IHttpRestClient *)mockHttpClient));
	mockHttpClient.Method(&IHttpRestClient::getResourcesAsJsonString)
		.Expect<0>("http://example.net/Devices")
		.Will(std::string(html));
